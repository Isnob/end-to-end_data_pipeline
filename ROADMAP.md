...
### Что изучить:
- [ ] **Библиотеки:** `ccxt` (криптовалюты) или `yfinance` (акции).
- [ ] **Валидация:** `Pydantic` для строгой проверки схем данных.
- [ ] **Контейнеризация:** `Docker` и `docker-compose` (базовые образы, сети, volumes).
- [ ] **БД:** Основы PostgreSQL (создание таблиц, индексы).
...
### Результат:
- В базе PostgreSQL копятся «сырые» данные в таблице `raw_assets`.
